...
const actions = {
  onBucketClickRow({dispatch}, item){
    dispatch('initS3', item)
  },
  onBucketItemSelected({commit, dispatch}, selected){
    if (Array.isArray(selected) && selected.length > 0) {
      dispatch('initS3', selected[0])
    } else {
      commit('updateSelected', [])
    }
  },
  onObjectClickRow({commit}, item){
    commit('setObjectSelection', item)
  },
  onObjectItemSelected({commit}, selected){
    if (Array.isArray(selected) && selected.length > 0) {
      commit('setObjectSelection', selected[0])
    } else {
      commit('updateObjectSelected', [])
    }
  },
