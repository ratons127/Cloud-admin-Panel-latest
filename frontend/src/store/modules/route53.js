...
const actions = {
  onClickHostZoneRow({dispatch}, item){
    dispatch('initTab', item)
  },
  onHostZoneSelected({commit, dispatch}, selected){
    if (Array.isArray(selected) && selected.length > 0) {
      dispatch('initTab', selected[0])
    } else {
      commit('updateSelectedHostZones', [])
    }
  },
  onClickRecordSetRow({commit}, item){
    commit('setRecordSetSelection', item)
  },
  onRecordSetSelected({commit}, selected){
    if (Array.isArray(selected) && selected.length > 0) {
      commit('setRecordSetSelection', selected[0])
    } else {
      commit('updateSelectedRecordSets', [])
    }
  },
