    ...
    <v-row no-gutters>
      <v-col cols="12" class="actionBlock">
        <div style='float: left;margin-left:5px;display: flex;'>
          <h2 style='margin-right: 5px;'>CloudWatch Logs</h2> <v-btn v-blur v-on:click="refresh" icon size="small"><v-icon>mdi-refresh</v-icon></v-btn>
          <v-text-field
            v-model="search"
            autocomplete="off"
        ...
        <div class="actions">
          <v-btn v-blur icon size="small" color="primary" style='margin-right: 10px;margin-left:20px;' @click="refreshEvents">
...
export default {
  ...
  methods: {
    onClickRow(event, row){
      ...
      this.$store.dispatch('cloudwatchlogs/onClickRow', item)
    },
    onItemSelected(selected){
      this.$store.dispatch('cloudwatchlogs/onItemSelected', selected)
    },
    selectTab(tabState){
