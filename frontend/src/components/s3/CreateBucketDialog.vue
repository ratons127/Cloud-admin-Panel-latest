<template>
  <v-dialog v-model="createBucketDialog" max-width="450">
    <template v-slot:activator="{ props }">
      <v-btn class="action-btn" color="primary"  size="small" v-bind="props">Create bucket</v-btn>
    </template>
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green-darken-1"  variant="text" @click="createBucketDialog = false">Cancel</v-btn>
        <v-btn color="green-darken-1"  variant="text" v-on:click="createBucket">Create</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
