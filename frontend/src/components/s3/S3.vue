  ...
  <v-container fluid>
    <v-row no-gutters>
      <v-col cols="12" class="actionBlock">
        <div style='float: left;margin-left:5px;display: flex;'>
          <h2 style='margin-right: 5px;'>S3</h2> <v-btn v-blur v-on:click="refresh" icon size="small"><v-icon>mdi-refresh</v-icon></v-btn>
          <v-text-field
            v-model="search"
    ...
    <v-row no-gutters>
      <v-col cols="3">
        <template>
          <v-data-table
            @click:row="onBucketClickRow"
            @update:selected="onBucketItemSelected"
...
export default {
  ...
  methods: {
    onBucketClickRow(event, row){
      const item = row && row.item ? row.item : event
      this.$store.dispatch('s3/onBucketClickRow', item)
    },
