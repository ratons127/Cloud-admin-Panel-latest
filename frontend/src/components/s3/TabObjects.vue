    ...
    <v-row no-gutters style='padding-left:25px;padding-top:15px;padding-left:25px;'>
      <v-col cols="12" v-if="selectedBucket.Name != null" style='margin-bottom:25px'>
        <div class="actions">
          ...
          <v-btn class="action-btn" color="primary" @click.prevent="downladObject"  size="small" v-if="showObjectActions">
      ...
      <v-col cols="12" style='margin-top:15px'>
        <div v-if="(objects.length > 0) && regionErrorMessage === ''">
          <v-data-table
            @click:row="onObjectClickRow"
            @update:selected="onObjectItemSelected"
            v-model:selected="selectedObjectArr"
...
export default {
  ...
  methods: {
    onObjectClickRow(event, row){
      const item = row && row.item ? row.item : event
      this.$store.dispatch('s3/onObjectClickRow', item)
    },
