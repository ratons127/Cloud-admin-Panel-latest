<template>
  <v-dialog v-model="deleteObjectDialog" max-width="1000" v-if="showBucketActions">
    <template v-slot:activator="{ props }">
      <v-btn class="action-btn"  size="small" color="error" v-bind="props">Delete object</v-btn>
    </template>
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green-darken-1"  variant="text" @click="deleteObjectDialog = false">Cancel</v-btn>
        <v-btn color="green-darken-1"  variant="text" v-on:click="deleteObject">Delete</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
