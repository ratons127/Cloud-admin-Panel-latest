    ...
    <v-card>
      ...
      <v-card-text style='height: 600px;overflow-y: auto;'>
        <v-row no-gutters style='margin-top:25px;'>
          ...
          <v-col cols="3"><v-btn  size="large" class="reset_btn" @click="resetNavMenu">Reset to default</v-btn></v-col>
        </v-row>
        <v-row no-gutters style='margin-top:25px;'>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green-darken-1"  variant="text" @click="navbarSettingsDialog = false">Close</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
<style>
.v-theme--dark .menuBlock > .v-input__control > .v-input__slot {
  background: white !important;
}
.v-theme--dark .menuBlock input {
