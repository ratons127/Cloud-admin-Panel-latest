<template>
  <v-dialog v-model="deleteRecordDialog" max-width="450" v-if="showActions">
    ...
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green-darken-1"  variant="text" @click="deleteRecordDialog = false">Cancel</v-btn>
        <v-btn color="green-darken-1"  variant="text" v-on:click="deleteRecord">Delete</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
