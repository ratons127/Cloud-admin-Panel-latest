<template>
  <v-dialog v-model="createRecordDialog" persistent max-width="450">
    <template v-slot:activator="{ props }">
      <v-btn class="action-btn createRecordSetBtn" color="primary"  size="small" v-bind="props">Create Record Set</v-btn>
    </template>
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green-darken-1"  variant="text" @click="createRecordDialog = false">Cancel</v-btn>
        <v-btn color="green-darken-1"  variant="text" v-on:click="createRecord">Create</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
