<template>
  <div v-if="selectedRepository.RepositoryName != null">
    ...
    <v-row no-gutters style='padding-left:25px;'>
      ...
      <v-col cols="12">
        <code style='color: black; padding-top: 12px;padding-bottom:12px;white-space: nowrap;display: block;overflow-y: hidden;'>aws ecr get-login-password --region {{region}} | docker login -u AWS --password-stdin {{selectedRepository.RepositoryUri}}</code>
      </v-col>
