<template>
  <v-dialog v-model="importDialog" max-width="1000" v-if="showActions">
    <template v-slot:activator="{ props }">
      <v-btn class="action-btn" color="primary"  size="small" v-bind="props">Import</v-btn>
    </template>
    <v-card>
      ...
      <v-card-text>
        <v-row no-gutters style='margin-top:25px;'>
          ...
          <v-col cols="12">
            <v-file-input @update:modelValue="onUploadCertificate" label="Certificate File"></v-file-input>
          </v-col>
          <v-col cols="12">
            <v-file-input @update:modelValue="onUploadPrivateKey" label="Private Key File"></v-file-input>
          </v-col>
          <v-col cols="12">
            <v-file-input @update:modelValue="onUploadFullChain" label="Full Chain File"></v-file-input>
          </v-col>
        </v-row>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green-darken-1"  variant="text" @click="importDialog=false">Cancel</v-btn>
        <v-btn color="green-darken-1"  variant="text" @click="importCerts">Import</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
