<template>
  <v-dialog persistent v-model="protectDialog" max-width="1000">
    <template v-slot:activator="{ props }">
      <v-btn class="action-btn"  size="small" :color="terminationProtection ? 'error' : 'success'" v-bind="props">{{terminationProtection ? "Unprotect" : "Protect"}}</v-btn>
    </template>
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green-darken-1"  variant="text" @click="protectDialog=false">Cancel</v-btn>
        <v-btn color="green-darken-1"  variant="text" @click="toggleTerminationProtection">{{terminationProtection ? "Unprotect" : "Protect"}}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
