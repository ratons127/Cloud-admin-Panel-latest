<template>
  <v-dialog v-model="createDialog" max-width="900" v-if="showActions">
    <template v-slot:activator="{ props }">
      <v-btn class="action-btn" color="primary"  size="small" v-bind="props">Create</v-btn>
    </template>
    <v-card>
      ...
      <v-card-actions>
        ...
        <v-btn color="green-darken-1"  variant="text" v-on:click="createKeypair">Create</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
