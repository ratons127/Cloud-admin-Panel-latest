    ...
    <v-row no-gutters>
      <v-col cols="12" class="actionBlock">
        <div style='float: left;margin-left:5px;display: flex;'>
          <h2 style='margin-right: 5px;'>Secrets Manager</h2> <v-btn v-blur v-on:click="refresh" icon size="small"><v-icon>mdi-refresh</v-icon></v-btn>
          <v-text-field
            v-model="search"
...
export default {
  ...
  methods: {
    onClickRow(event, row){
      ...
      this.$store.dispatch('secretsmanager/onClickRow', item)
    },
    onItemSelected(selected){
      this.$store.dispatch('secretsmanager/onItemSelected', selected)
    },
    refresh(){
