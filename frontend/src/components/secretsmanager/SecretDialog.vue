<template>
  <v-dialog v-model="secretValueDialog" max-width="900" v-if="showActions">
    <template v-slot:activator="{ props }">
      <v-btn class="action-btn" color="primary"  size="small" v-bind="props">View secret</v-btn>
    </template>
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green-darken-1"  variant="text" @click="secretValueDialog = false">Close</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
