<template>
  <v-dialog v-model="stopDialog" max-width="900" v-if="showActions">
    <template v-slot:activator="{ props }">
      <v-btn class="action-btn" color="primary"  size="small" v-bind="props">Stop</v-btn>
    </template>
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green-darken-1"  variant="text" @click="stopDialog = false">Cancel</v-btn>
        <v-btn color="green-darken-1"  variant="text" v-on:click="stopInstance">Stop</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
