<template>
  <v-dialog v-model="forceUpdateDialog" max-width="1000" v-if="showActions">
    <template v-slot:activator="{ props }">
      <v-btn class="action-btn" color="primary"  size="small" v-bind="props">Force update</v-btn>
    </template>
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green-darken-1"  variant="text" @click="forceUpdateDialog = false">Cancel</v-btn>
        <v-btn color="green-darken-1"  variant="text" v-on:click="forceUpdate">Force Update</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
